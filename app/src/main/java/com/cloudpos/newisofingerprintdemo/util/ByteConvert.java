...
//      if (bigIndian) {
    ...
    public static byte[] subByteArray(byte[] byteArray, int length) {
        ...
        System.arraycopy(byteArray, 0, arrySub, 0, length);
        return arrySub;
    }

    public static byte[] subByteArray(byte[] byteArray, int offset, int length) {
        byte[] arrySub = new byte[length];
        System.arraycopy(byteArray, offset, arrySub, 0, length);
        return arrySub;
    }
}
